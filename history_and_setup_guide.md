...
* **Ubuntu 24.04 LTS의 Socket-Based Activation 메커니즘 대응**:
  기존 Ubuntu 20.04 이전 버전에서는 `sshd` 서비스가 독립 프로세스로 상시 구동되어 `sudo systemctl restart ssh`만으로 변경 사항이 반영되었으나, **Ubuntu 24.04 LTS부터는 리소스 효율화를 위해 systemd 소켓 기반 호출(Socket-Based Activation) 방식**으로 동작 구조가 변경되었다.
...
## Chapter 8. 2026-08-13 시놀로지 10TB 팀 전용 스토리지 구축, Tailscale P2P Mesh VPN 구축 및 팀원 안내서 완성

### 8.1 배경 및 과제 정의
시놀로지 55TB 실효 용량 중에서 팀원 10명을 위한 **10TB 전용 스토리지 영역**을 세분화하여, 5TB 팀 공용 공유 폴더(`00_Asia_hub`)와 5TB 팀원 개인 독립 방(1인당 500GB `/home`)으로 안전하게 분할 구축하고, 외부망에서도 터미널 명령어 없이 원클릭 접속이 가능한 Tailscale P2P Mesh VPN 네트워크를 완성하는 과제를 수행하였다.

### 8.2 핵심 기술 의사결정 및 시행착오 해결
1. **공유 폴더 Quota vs 사용자 홈 Quota 격리 구조 확립**:
   * DSM 유저 개별 편집 창의 [할당량] 탭은 `00_Asia_hub` 공유 폴더 전용 제한 칸이므로, 개인 방(`home`) 500GB 제한을 걸기 위해 **[제어판 ➡️ 사용자 및 그룹 ➡️ 고급 탭 ➡️ 사용자 홈 ➡️ 사용자 홈 할당량 활성화 (500GB)]** 경로를 사용하여 공용 5TB 공간 차감 없이 순수 개인 방만 500GB로 깔끔하게 분리 구축 완료.
2. **Tailscale P2P Mesh VPN 구축 (Synology NAS IP: `100.118.194.54`)**:
   * 시놀로지 NAS 패키지 센터에서 Tailscale 설치 및 공유 계정 인증 완료.
   * 팀원 PC에서 터미널/SSH 접속 없이, 윈도우용 Tailscale 앱 1회 로그인만으로 전 세계 어디서든 `\\100.118.194.54\00_Asia_hub` (Z: 드라이브) 및 `\\100.118.194.54\home` (Y: 드라이브)에 직결되도록 네트워크 인프라 완성.
3. **윈도우 자격 증명 및 `#recycle` 보안 매커니즘**:
   * 윈도우 탐색기 연결 시 `다른 자격 증명을 사용하여 연결` 및 `내 자격 증명 기억`을 적용하여 포맷 전까지 자동 로그인 환경 완성.
   * 실수 삭제를 대비한 `#recycle` (휴지통) 활성화 및 일반 사용자 비인가 엑세스 차단 정책 적용.

### 8.3 결과 및 연동 배포
팀원들이 바로 참고할 수 있는 1페이지 사용 설명서인 [team_user_guide.md](team_user_guide.md) 문서 작성을 완료하고 레포지토리에 반영하였다.
